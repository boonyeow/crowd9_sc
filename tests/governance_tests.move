...
module crowd9_sc::governance_tests {
    ...
    fun division_test() {
        ...
        let share1 = coin_value / vault.total_supply_value * balance::value(&vault.balance);
        let share2 = (((coin_value as u256) * (balance::value(
            &vault.balance
        ) as u256) / (vault.total_supply_value as u256)) as u64);

        let share3 = lib::mul_div_u64(coin_value, balance::value(&vault.balance), vault.total_supply_value);
        debug::print(&share3);
        debug::print(&share1);
        debug::print(&share2);

        debug::print(&lib::mul_div_u64(1, 67, 100));
